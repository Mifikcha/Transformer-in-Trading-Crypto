...
# Сохранение обновлённого датасета (теперь с базовыми таргетами)
output_file = 'btcusdt_5m_with_targets_base.csv'
df.to_csv(output_file, index=True)
print(f"\nДатасет с базовыми таргетами сохранён как '{output_file}'")

# Продолжение после шага 4 (df уже загружен из 'btcusdt_5m_with_targets_base.csv')

# Из шага 3 — значения c
c_scenarios = {
    'optimistic': 0.0010,
    'base':       0.0019,
    'pessimistic':0.0020
...
coverage_data = {
    name: df[f'trading_class_{name}'].value_counts(normalize=True).get(['long', 'short'], 0).sum() * 100
...
# Сохранение финального датасета с таргетами
final_file = 'btcusdt_5m_final_with_targets.csv'
df.to_csv(final_file, index=True)
print(f"\nФинальный датасет с базовыми и торговыми таргетами сохранён как '{final_file}'")